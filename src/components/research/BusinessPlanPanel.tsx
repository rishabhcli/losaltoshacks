    ...
    <div className="flex justify-center py-1">
      <div
        className={`w-0.5 h-4 rounded-full transition-colors ${
          active ? "bg-gradient-to-b from-blue-400 to-blue-600" : "bg-slate-200"
        }`}
      />
    </div>
  );
...
export function BusinessPlanPanel({ plans, agents, discoveries, missionPrompt, isRunning, finalOptions, onStopAll }: Props) {
  ...
  return (
    <div className="flex flex-col h-full gap-0 overflow-hidden">
      ...
      <div className="flex-shrink-0 overflow-auto p-3 border-b border-slate-100 space-y-0">
        {/* Mission node */}
        ...
        <div className="flex gap-1.5 justify-center flex-wrap">
          {AGENTS.map((def) => {
            ...
            return (
              <div key={def.id} className="flex flex-col items-center gap-0.5 flex-1 min-w-[48px] max-w-[68px] px-1.5 py-1.5 rounded-md border border-slate-100 bg-white/60">
                ...
                {count > 0 && <span className="text-[8px] text-slate-500 bg-slate-100 px-1 rounded">{count}</span>}
              </div>
            );
          })}
        </div>
        ...
        {/* Synthesis status */}
        {latest && (
          <Card className={`border ${latest.is_final ? "border-green-200 bg-green-50/50" : "border-blue-200 bg-blue-50/30"}`}>
            <CardContent className="p-2.5">
        ...
        {latest && (
          <div className="grid grid-cols-2 gap-1.5">
            {PLAN_SECTIONS.map((section) => {
              ...
              return (
                <div key={section.key} className={`rounded-md border p-2 ${filled ? "border-blue-200/80 bg-white" : "border-slate-100 bg-slate-50/50"}`}>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2">
            ...
            {latest && <Badge variant="secondary" className="text-[9px] bg-blue-50 text-blue-600">v{latest.version}</Badge>}
          </div>
          {latest?.raw_plan && (
        ...
        {/* Version history */}
        {history.length > 0 && (
          <div className="flex items-center gap-1.5 px-3 py-1.5 border-t border-slate-100 shrink-0 flex-wrap">
            <span className="text-[9px] text-slate-400 uppercase tracking-wider mr-0.5">History</span>
            {history.map((plan) => (
              <Badge key={plan._id} variant="outline" className="text-[9px] text-slate-500 border-slate-200 py-0">
        ...
        {/* Action buttons */}
        <div className="px-3 py-2 shrink-0 flex gap-2 border-t border-slate-100">
          ...
          {isRunning && (
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-blue-200 bg-blue-50 text-blue-700 text-xs font-medium hover:bg-blue-100 transition-colors"
            >
              <ExternalLink className="w-3 h-3" />
